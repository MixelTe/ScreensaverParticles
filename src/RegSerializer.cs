...
public class RegSerializer
{
	public static void Save(string keyName, object settings)
	{
		...
		foreach (var fi in t.GetFields())
		{
			...
			if (n.StartsWith("DEV") || v == null) continue; //Don't save settings with "DEV" prefix
	...
	public static void Load(string keyName, object settings)
	{
		...
		foreach (var fi in t.GetFields())
		{
			...
			if (!string.IsNullOrWhiteSpace(s))
			{
				...
				{
					var parts = s.Split(',');
					if (parts.Length != 4) throw new Exception($"Value of field is corrupted! Name: {n} Type: {n}");
