...
class ScreensaverControllerNew : IMainController
{
	...
	public ScreensaverControllerNew()
	{
		...
		var w = Screen.PrimaryScreen?.Bounds.Width ?? 100;
		var h = Screen.PrimaryScreen?.Bounds.Height ?? 100;
		...
		_game = game;
		_painter = new Painter(game, new Rectangle(0, 0, w, h));
	}
	...
	public void Start()
	{
		if (Program.Settings.DEV_Presentation && TargetControl != null) TargetControl.Visible = false;
		_window?.Create();
	}
...
class GOGraphics(GameOverlay.Drawing.Graphics gfx) : IGraphics
{
	private GameOverlay.Drawing.Graphics? _gfx = gfx;
	private GameOverlay.Drawing.SolidBrush? _brush = gfx.CreateSolidBrush(0, 0, 0);
